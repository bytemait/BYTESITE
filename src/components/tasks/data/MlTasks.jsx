...
const TaskCard = ({
	...
	return (
		<motion.li
			...
			whileHover={{
				scale: [1.02, 1.04],
				boxShadow: "0 12px 30px #00ffae88",
			}}
		...
		>
				...
				<div className="flex items-center gap-3 sm:gap-4">
					...
					<div className="min-w-0">
						...
						<div className="flex flex-wrap gap-2 sm:gap-3 mt-1 text-xs sm:text-sm text-gray-300 font-mono items-center">
							<span
								className={`px-2 sm:px-3 py-1 rounded-full text-white text-xs ${difficultyColors[difficulty] || difficultyColors.basic}`}
							>
								{difficulty}
							</span>
			...
			{datasetInfo && (
				<div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-gray-800 bg-opacity-50 rounded-lg border-l-4 border-[#00ffae]">
					<h4 className="text-[#00ffae] font-semibold mb-2 text-sm sm:text-base">
						Dataset:
					</h4>
					...
					>
						{steps && (
							<section className="mb-4 sm:mb-5">
								<h4 className="text-[#00ffae] font-bold mb-2">
									Steps
								</h4>
						...
						{extras && <section className="mb-2">{extras}</section>}
					</motion.div>
				)}
			</AnimatePresence>
		</motion.li>
		...
		<div className="relative min-h-screen p-4 sm:p-6 md:p-8 lg:p-16 text-white">
			...
			>
				...
				<p className="mt-4 sm:mt-6 text-base sm:text-lg md:text-xl max-w-xs sm:max-w-2xl md:max-w-3xl mx-auto text-gray-300 leading-relaxed">
					Explore the full spectrum of ML: from traditional
					classification and deep learning to cutting-edge GenAI with
					RAG systems. Build, train, and deploy intelligent solutions.
				</p>
			...
			<motion.ul
				layout
				className="space-y-8 sm:space-y-12 md:space-y-16 max-w-sm sm:max-w-2xl md:max-w-3xl lg:max-w-4xl mx-auto"
			>
				{tasks.map((task, idx) => (
